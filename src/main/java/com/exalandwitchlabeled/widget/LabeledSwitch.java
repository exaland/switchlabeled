package com.exalandwitchlabeled.widget;

/*
 * Copyright (C) 2025 Alexandre MAGNIER
 *
...
/**
 * <p>
 * Created by Alexandre MAGNIER on 5-12-2025
 * </p>
 */
...
public class LabeledSwitch extends ToggleableView {
    ...
    private void initView() {
        this.isOn = false;
        this.labelOn = "EN LIGNE";
        this.labelOff = "HORS LIGNE";

        this.enabled = true;
        this.textSize = (int)(12f * getResources().getDisplayMetrics().scaledDensity);

        colorBorder = colorOn = ContextCompat.getColor(getContext(), R.color.colorExalandRed);
        ...
        try {
            ...
            for (int i = 0; i < N; ++i) {
                ...
                if (attr == R.styleable.Toggle_on) {
                    ...
                    int accentColor = ContextCompat.getColor(getContext(), R.color.colorExalandRed);
                    ...
                    // Preserve the value set in initView() as default (usually true)
                    enabled = tarr.getBoolean(R.styleable.Toggle_android_enabled, enabled);
                }
            }
        } finally {
            if (tarr != null) tarr.recycle();
        }
    }
...
//      Drawing Switch Thumb here
    ...
    @Override public final boolean performClick() {
        Log.d("LabeledSwitch", "performClick() called - current isOn=" + isOn);
        super.performClick();
        if (isOn) {
        ...
        isOn =! isOn;
        Log.d("LabeledSwitch", "State changed by user interaction to isOn=" + isOn + " — notifying listener");
        notifyToggled();
        return true;
    }
    ...
    @Override public final boolean onTouchEvent(MotionEvent event) {
        Log.d("LabeledSwitch", "onTouchEvent: action=" + event.getAction() + " x=" + event.getX() + " enabled=" + isEnabled());
        if(isEnabled()) {
            float x = event.getX();
    ...
    @Override public void setOn(boolean on) {
        ...
        invalidate();
        // Notify listener when state is changed programmatically
        Log.d("LabeledSwitch", "setOn() programmatic change - notifying listener with isOn=" + isOn);
        notifyToggled();
    }
