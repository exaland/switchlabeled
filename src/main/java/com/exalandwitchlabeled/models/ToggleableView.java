package com.exalandwitchlabeled.models;
...
import com.exalandwitchlabeled.design.OnToggledListener;
...
public class ToggleableView extends View {
    ...
    // Track the last state that was reported to the listener to avoid duplicate callbacks
    protected Boolean lastNotifiedState = null;
    ...
    /**
     * Notify the registered OnToggledListener if the state changed since last notification.
     * This prevents duplicate callbacks when the state is set programmatically and when
     * a listener is attached after the state was already set.
     */
    protected void notifyToggled() {
        if (onToggledListener != null) {
            if (lastNotifiedState == null || lastNotifiedState != isOn) {
                onToggledListener.onSwitched(this, this.isOn);
                lastNotifiedState = this.isOn;
            }
        }
    }
    ...
    public void setOnToggledListener(OnToggledListener onToggledListener) {
        ...
        // Notify listener with current state (only if it wasn't already notified for this state)
        notifyToggled();
    }
}